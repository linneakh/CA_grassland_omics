...
cts.0 <- read.csv("../output/metaT/final_count_KO_f.csv", header = TRUE) %>% dplyr::select(-X) 
...
#subset to t3
coldata.t3 <- coldata.final %>%
  filter(str_detect(rownames(.), "t3")) 

limit = round(nrow(coldata.t3)*0.2)

cts.t3 <- cts.final %>%
  as.data.frame() %>%
  dplyr::select(contains("t3")) %>%
  mutate(sum.count = rowSums(.)) %>%
  mutate(gr.zero = rowSums(. > 0)) %>%
  filter(gr.zero > limit) %>%
  mutate(sum.count.per = sum.count/sum(sum.count)) %>%
...
#subset to t48
coldata.t48 <- coldata.final %>%
  filter(str_detect(rownames(.), "t48")) 

limit = round(nrow(coldata.t48)*0.2)

cts.t48 <- cts.final %>%
  as.data.frame() %>%
  dplyr::select(contains("t48")) %>%
  mutate(sum.count = rowSums(.)) %>%
  mutate(gr.zero = rowSums(. > 0)) %>%
  filter(gr.zero > limit) %>%
  mutate(sum.count.per = sum.count/sum(sum.count)) %>%
...
###create a deseq2 object with Moisture and timepoint
coldata.t3$Moisture <- factor(coldata.t3$Moisture, c("x50", "x100"))

coldata.t3$Moisture <- relevel(coldata.t3$Moisture, ref = "x50")


dds.t3 <- DESeqDataSetFromMatrix(countData = cts.t3,
                              colData = coldata.t3,
                              design= ~ Moisture)
...
```{r data_exploration}
vstcounts <- vst(dds.t3, nsub=40, blind=TRUE)
...
# export VST transformed table to be used for downstream analyses
write.csv(vst.export, "../output/metaT/vst-ko-t3.csv")
...
```{r}
dds.t3.object <- DESeq(dds.t3)
resultsNames(dds.t3.object) # lists the coefficients

res <- results(dds.t3.object, contrast = c("Moisture","x100","x50"))
plotMA(res, ylim=c(-40,40))
...
write.csv(res, "../../output/metaT/deseq2_t3_res.csv")
```

```{r create_deseq2_object_reference_LD}

###create a deseq2 object with Moisture and timepoint
coldata.t48$Moisture <- factor(coldata.t48$Moisture, c("x50", "x100"))

coldata.t48$Moisture <- relevel(coldata.t48$Moisture, ref = "x50")


dds.t48 <- DESeqDataSetFromMatrix(countData = cts.t48,
                              colData = coldata.t48,
                              design= ~ Moisture)
...
```{r data_exploration}
vstcounts <- vst(dds.t48, nsub=40, blind=TRUE)
...
# export VST transformed table to be used for downstream analyses
write.csv(vst.export, "../output/metaT/vst-ko-t48.csv")
...
```{r}
dds.t48.object <- DESeq(dds.t48)
resultsNames(dds.t48.object) # lists the coefficients

res <- results(dds.t48.object, contrast = c("Moisture","x100","x50"))
plotMA(res, ylim=c(-40,40))
...
write.csv(res, "../../output/metaT/deseq2_t48_res.csv")
```