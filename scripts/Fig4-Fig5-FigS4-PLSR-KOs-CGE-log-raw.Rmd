...
source("../scripts/functions/PLSR-functions.R")
...
cue.only <- cue %>% dplyr::select(SampleID, CUE.g.dry.um)
...
cue.all <- cue.only %>%
  ...
  mutate(CUE.metric.log = log(CUE.g.dry.um)) %>%
  dplyr::select(-CUE.g.dry.um)
...
d_bio_aa    <- make_mean_table(cue.timepoint, vip_kos, VIP_aa_bio)
d_bio_lipid <- make_mean_table(cue.timepoint, vip_kos, VIP_lipid_bio)
d_bio_nuc   <- make_mean_table(cue.timepoint, vip_kos, VIP_nuc_bio)
d_bio_wall  <- make_mean_table(cue.timepoint, vip_kos, VIP_wall_bio)
d_bio_eps <- make_mean_table(cue.timepoint, vip_kos, VIP_eps_bio)
d_bio_teich <- make_mean_table(cue.timepoint, vip_kos, VIP_teich_bio)
...
# bio_all <- make_mean_table(cue.timepoint, vip_kos, all_bio_kos)
# eng_all <- make_mean_table(cue.timepoint, vip_kos, all_eng_kos)
...
```{r biosynthesis and energy Figure 5}
...
bottom_row <- wrap_plots(annotated[5:6], nrow = 1)
...
bottom_row <- wrap_plots(annotated[7:9], nrow = 1)

final_plot_supp <- (top_row / middle_row / bottom_row) + 
              plot_layout(guides = 'collect') + 
              plot_annotation(tag_levels = 'a') +
              theme(legend.position = 'bottom')
...
ggsave(file.path(out_dir, "CGE_vs_expression_grid_10.png"), plot = final_plot, width = 10, height = 12, units = "in", dpi = 300)
ggsave(file.path(out_dir, "CGE_vs_expression_grid_10.pdf"), plot = final_plot, width = 10, height = 12, units = "in", dpi = 300)

ggsave(file.path(out_dir_2, "CGE_vs_expression_grid_co2_10_supp.png"), plot = final_plot_supp, width = 13, height = 12, units = "in", dpi = 300)
ggsave(file.path(out_dir_2, "CGE_vs_expression_grid_co2_10_supp.pdf"), plot = final_plot_supp, width = 13, height = 12, units = "in", dpi = 300)

ggsave(file.path(out_dir_deg, "CGE_vs_expression_grid_deg_10.png"), plot = final_plot_deg, width = 12, height = 6, units = "in", dpi = 300)
ggsave(file.path(out_dir_deg, "CGE_vs_expression_grid_deg_10.pdf"), plot = final_plot_deg, width = 12, height = 6, units = "in", dpi = 300)
