...
make_correlation_plots_with_legend <- function(df,x_label="vst VST") {
  p <- ggplot(data = df, aes_string(x = "sum.vst", y = "CUE.metric.log", color = "Timepoint", shape = "Moisture")) +
    geom_point(size = 3) +
    scale_color_viridis(discrete = TRUE,
                        direction = -1,
...
annotate_panels_with_fdr <- function(p_list, data_list, x = "sum.vst", y = "CUE.metric.log", bottom_row_indices = NULL) {
  ...
  panel_letters <- letters[seq_along(p_list)]
  
  # Annotate each panel with its adjusted p-value, optional panel label, and hide x-axis labels except bottom row
  annotated_plots <- mapply(function(p, dat, pval, idx, letter) {
    xv <- dat[[x]]; yv <- dat[[y]]
    ok <- is.finite(xv) & is.finite(yv)
    n_ok <- sum(ok)
    safe_range <- function(v) {
    ...
    p
  }, p_list, data_list, adj_p, seq_along(p_list), panel_letters, SIMPLIFY = FALSE)
