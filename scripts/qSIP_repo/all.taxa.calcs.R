...
all.taxa.calcs <- function(X.all, comparisons, M.soil=NULL, vars=c("taxon", "density.g.ml", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", prop.O.from.water=0.50, v.frac=50, copies.cell=6, pgC.cell=0.1, CI=0.90, draws=1000, tailed.test=1){
  ...
  #dir.create(path=paste("/qSIP_output", sep=""), showWarnings=FALSE)
  write.table(wad1.boots, paste(Dir.o,"bootstrapped_wad1.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  write.table(wad2.boots, paste(Dir.o,"bootstrapped_wad2.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  write.table(wad.diff.boots, paste(Dir.o,"bootstrapped_wad_diff.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  write.table(ape.boots, paste(Dir.o,"bootstrapped_ape.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  if (!sham){   #If M.soil was not a sham, then provide this output:
    write.table(r.boots, paste(Dir.o,"bootstrapped_r.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
    write.table(C.flux.boots, paste(Dir.o,"bootstrapped_C_fluxes.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
    write.table(N1.boots, paste(Dir.o,"bootstrapped_N1.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)    
    write.table(N2.boots, paste(Dir.o,"bootstrapped_N2.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)    
    write.table(N.boots, paste(Dir.o,"bootstrapped_N.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)    
  }
