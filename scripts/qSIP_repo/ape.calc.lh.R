### Given two treatments, returns an estimate (with uncertainty) of excess atom fraction (or atom percent excess) of the heavy isotope
#
#     output = ape.calc(X.light, X.heavy, boot.out.light, boot.out.heavy, MW.out, iso.compare, var="trt.code", CI=0.90)
#
...
#
#     notes:          requires the functions 'WAD.func', 'boot.WAD.func', 'MW.calc', & 'comparison.message'
...
ape.calc <- function(X.light, X.heavy, boot.out.light, boot.out.heavy, MW.out, iso.compare, var="trt.code", CI=0.90){
  ...
  obs.ape=ifelse(obs.ape<0, 0, ifelse(obs.ape>1, 1, obs.ape))
  boot.apes=ifelse(boot.apes<0, 0, ifelse(boot.apes>1, 1, boot.apes))
