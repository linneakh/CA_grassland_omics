...
all.taxa.calcs.pop <- function(X.all, comparisons, M.soil, vars=c("taxon", "copies", "tube", "trt.code", "g.soil"), growth.model="exponential", vol=c(100, 50), copies.cell=6, pgC.cell=0.1, CI=0.90, draws=1000){
  ...
  write.table(net.r.boots, paste(Dir.o,"bootstrapped_r_pop.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  write.table(net.C.flux.boots, paste(Dir.o,"bootstrapped_C_fluxes_pop.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  write.table(N.T0.boots, paste(Dir.o,"bootstrapped_N_T0_pop.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
  write.table(N.Tt.boots, paste(Dir.o,"bootstrapped_N_Tt_pop.txt", sep=""), append=F, quote=F, sep="\t", eol="\n", na="NA", dec=".", row.names=F, col.names=T)
