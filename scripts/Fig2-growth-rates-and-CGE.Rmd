...
source.df <- read.csv("../data/qSIP/sample_source_biomass_info.csv", header = TRUE) %>% select(-X)

#load soil moisture data
sm.0 <- read.csv("../data/soil_moisture.csv")

#load SIP efficiency
SIP.0 <- read.csv("../data/qSIP/qPCR_efficiency.csv")
...
df.final.gr.lenient.CUE <- df.final.gr.lenient %>%
  ...
  mutate(cum.growth.copies.dry.g = b.boot.median.dry * (timepoint.num/24)) %>% #cumulative new growth
  ...
  mutate(CUE.g.dry = cum.growth.copies.dry.g / CO2.moles.g.dry) %>% #units: 16S copies/CO2 moles
  mutate(CUE.g.dry.um = CUE.g.dry / 10^6) %>% #units: 16S copies/CO2 umol
  mutate(biomass.C.g.dry.g = ((cum.growth.copies.dry.g * 10) / (5 * 10^15))) %>% #10 fg C / microbe; average 5 copies of 16S per microbe
...
#####bnet growth
#all rewet time points
m <- lmer(cum.growth.copies.dry.g ~ moisture * timepoint + (1|plot), data = df.final.gr.lenient.CUE.2)
anova(m)
#Type III Analysis of Variance Table with Satterthwaite's method
#                       Sum Sq    Mean Sq NumDF DenDF F value    Pr(>F)    
...
plot <- df.final.gr.lenient.CUE %>%
  ...
  filter(timepoint != "t0") %>%
  group_by(moisture, timepoint) %>%
  summarise(mean = mean(cum.growth.copies.dry.g),
            sd = sd(cum.growth.copies.dry.g),
            se = sd/sqrt(n())) %>%
  mutate(log.mean = log10(mean)) %>%
   mutate(timepoint = as.numeric(str_remove(timepoint, "t"))) %>%
  ggplot(aes(x=timepoint, y = mean, color = moisture)) +
  geom_point(size = 2) +
  ylab(bquote('cumulative new biomass (16S copies '* ~g-soil^-1*')')) +
  xlab("timepoint (h)") +
...
#CUE  (16S copies / umol CO2)
plot <- df.final.gr.lenient.CUE.2 %>%
  mutate(moisture = as.factor(moisture)) %>%
  group_by(moisture, timepoint) %>%
  summarise(mean = mean(CUE.g.dry.um),
            sd = sd(CUE.g.dry.um),
            se = sd/sqrt(n())) %>%
   mutate(timepoint = as.numeric(str_remove(timepoint, "t"))) %>%
  ggplot(aes(x=timepoint, y = mean, color = moisture)) +
  ...
  geom_errorbar(aes(ymin = mean-se, ymax = mean+se),
                width = 4) +
  ylab(expression(CGE~"(" * "16S copies"~mu*mol~CO[2]^{-1} * ")")) +
  #ylab(bquote('cumulative new biomass (16S copies '* ~g-soil^-1*')')) +
  scale_y_log10() +
  geom_smooth(linewidth = 0.25, method = "lm", formula = "y ~ log(x)", se=FALSE, size=1) +
  scale_color_manual(values = c(moisture_col_list)) +
  theme_bw() +
   theme(panel.grid.major = element_blank(),
    panel.grid.minor = element_blank())
 plot
ggsave("../Figures/Fig2-growth-rates-CGE/CGE-biomass-CO2.png", dpi = 300, width = 4, height = 4, units = "in")
ggsave("../Figures/Fig2-growth-rates-CGE/CGE-biomass-CO2.pdf", dpi = 300, width = 4, height = 4, units = "in")

#CUE  (log 16S copies / umol CO2)
plot <- df.final.gr.lenient.CUE.2 %>%
  mutate(moisture = as.factor(moisture)) %>%
  group_by(moisture, timepoint) %>%
  summarise(mean = mean(CUE.g.dry.um),
